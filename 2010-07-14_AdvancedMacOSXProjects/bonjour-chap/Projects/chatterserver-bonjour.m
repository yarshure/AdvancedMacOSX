...
/* compile with:
gcc -g -Wall -std=c99 -framework Foundation -o chatterserver-bonjour chatterserver-bonjour.m
*/
...
static NSString *const kChatterServerName = @"ChatterServer";
@interface ChatterDelegate : NSObject {
 @public
    NSNetService *service;
}
- (void)publishPort:(in_port_t)port;
@end
...
typedef uint8_t
READ_BUFFER_SIZE_not_less_than_MAX_MESSAGE_SIZE
...
main(int argc, char *argv[]) {
    ...
    err = getsockname(listen_fd, (struct sockaddr *)&ss, &len);
    if (-1 == err) {
        perror("getsockname");
        return EXIT_FAILURE;
    }
    ...
    NSLog(@"Creating delegate...");
    ChatterDelegate *delegate = [[ChatterDelegate alloc] init];
    [delegate publishPort:kPortNumber];
    NSLog(@"Delegate published.");
    ...
    while (true) {
        ...
        //NSLog(@"select returned: %d", nready);
        ...
        //NSLog(@"Did loop...");
        NSNetService *service = delegate->service;
        //NSLog(@"Service - name %@, type %@, delegate %@, addresses %@, port %ld", [service name], [service type], [service delegate], [service addresses], (long)[service port]);
    }
...
@implementation ChatterDelegate
- (void)publishPort:(in_port_t)port {
    service  = [[NSNetService alloc]
                         initWithDomain:@"local."
                         type:@"_chatter._tcp."
                         name:kChatterServerName
                         port:port];
    NSLog(@"%s: service: %@", __func__, service);
...
- (void)netService:(NSNetService *)sender didNotPublish:(NSDictionary *)dict {
    NSLog(@"%s: *** Did NOT publish: %@: %@", __func__, sender, dict);
}

- (void)netServiceDidStop:(NSNetService *)sender {
    NSLog(@"%s: Stopped: %@", sender);
}
@end
