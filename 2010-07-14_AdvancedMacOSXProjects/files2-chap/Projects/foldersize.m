// foldersize.m -- calculate the size of a folder
...
//gcc -g -Wmost -o foldersize foldersize.m
...
off_t sizeForFolderAttr (char *path) {
    ...
    if (fd == -1) {
        fprintf (stderr, "could not open directory '%s'.  Error %d %s\n",
                 path, errno, strerror(errno));
        goto bailout;
    }

    // a rough guess on the appropriate buffer size
    char attrbuf[ENTRIES_COUNT * (sizeof(fileinfo) + AVG_NAME_GUESSTIMATE)];
    ...
    while (1) {
        ...
        for (; count > 0; count--) {
            ...
            // Move to the next attribute in the returned set.
...
int main (int argc, char *argv[]) {
    // sanity check the program argument first
    if (argc != 3) {
