...
//gcc -g -o permtype-getattrlist permtype-getattrlist.m
...
void displayInfo (const char *filename) {
    ...
    // Clear out the attribute request structure first, otherwise
    ...
    // The returned data cannot be larger than the size of
    // PermTypeAttributes.
    ...
    // but blindly following pointers can hurt.
    ...
    if ((permAttributes.accessMask & S_ISUID) == S_ISUID) {
        printf ("  set-uid!\n");
    }   
    if ((permAttributes.accessMask & S_ISGID) == S_ISUID) {
        printf ("  set-group-id!\n");
    }
