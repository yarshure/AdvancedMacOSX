...
// Make our getters and setters.
@synthesize runQueue;
@synthesize concurrentExecution;
...
- (NSArray *) contents {
    ...
    NSString *contentsPath = [bundle pathForResource: @"contents"
                                     ofType: @"txt"];

    // Read the file and turn it into an array of strings split
    // on newlines.
    NSString *contentsString;
    contentsString = [NSString stringWithContentsOfFile: contentsPath
    ...
    // string.
    int count = [mutableContents count];
    if ([[mutableContents objectAtIndex: count - 1] length] == 0) {
    ...
    int i;
    for (i = 0; i < count; i++) {
        int newIndex = random() % count;
        [mutableContents exchangeObjectAtIndex: i
...
- (IBAction) start: (id) sender {
    ...
    for (i = 0; i < max; i++) {
        ...
        NSString *urlString = [contents objectAtIndex: i];
        NSURL *url = [NSURL URLWithString: urlString];

        // Make a new operation to download that URL
        SnarfOperation *op;
        op = [[SnarfOperation alloc] initWithURL: url];
...
- (void) observeValueForKeyPath: (NSString *) keyPath
    ...
    if ([keyPath isEqualToString: @"isFinished"]) {
        ...
        // Update concurrentExecution to reflect to the number
        // of operations currently running.  A status line text field
        // is bound to concurrentExecution.
