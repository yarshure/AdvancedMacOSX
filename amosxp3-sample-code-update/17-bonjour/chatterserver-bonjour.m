// chatterserver-bonjour.m -- chat server using standard sockets API, bonjour-enabled

// clang -g -Weverything -Wno-unused-parameter -framework Foundation -o chatterserver-bonjour chatterserver-bonjour.m
...
static ChatterUser *AcceptConnection (int acceptedFd) {
    ChatterUser *newUser = NULL;
    struct sockaddr_storage addr;
    socklen_t addr_len = sizeof(addr);

    int clientFd = acceptedFd;
    int result = getpeername (clientFd, (struct sockaddr *)&addr, &addr_len);
...
static void RegisterRunLoopSocket (int listenFd, CFSocketCallBack callback,
                                   BOOL listen, ChatterUser *user) {
    CFSocketContext context = { .info = user };
...
} // RegisterRunLoopSocket


static void userCallback (CFSocketRef s,
                          CFSocketCallBackType type,
                          CFDataRef address,
                          const void *data,
                          void *info) {
    NSLog (@"user callback!");
    HandleRead ((ChatterUser *)info);
} // userCallback


static void listenCallback (CFSocketRef s,
                            CFSocketCallBackType type,
                            CFDataRef address,
                            const void *data,
                            void *info) {
    CFSocketNativeHandle fd = *(CFSocketNativeHandle *) data;
    ChatterUser *user = AcceptConnection (fd);
    RegisterRunLoopSocket (fd, userCallback, NO, user);
} // litenCallback
...
int main (int argc, char *argv[]) {
    ...
    if (argc == 1) {
        fprintf (stderr, "usage: %s <servicename> [ipv6]\n", argv[0]);
        return EXIT_FAILURE;
    }

    int exit_status = EXIT_FAILURE;
    const bool useIPv6 = (argc > 2);
    ...
    RegisterRunLoopSocket (listenFd, listenCallback, YES, NULL);
    ...
    NSNetService *netService = 
        [[NSNetService alloc] initWithDomain: @""
                              ...
                              name: serviceName
                              port: kPortNumber];
    NetServiceDelegate *delegate 
        = [[[NetServiceDelegate alloc] init] autorelease];
